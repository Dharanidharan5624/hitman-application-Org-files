...
def save_version_file(version_data=None):
    if not CONFIG_TARGET_DIR:
        return False
    version_file_path = os.path.join(CONFIG_TARGET_DIR)
    try:
...
def create_user_and_dbs(info):
    ...
    try:
        ...
        try:
            ...
            if e.errno == errorcode.ER_CANNOT_USER:print_status(f"User {info['user']} already exists", "SUCCESS")
            else:
                raise
...
class ConfirmationPage(tk.Frame):
    ...
    def confirm_installation(self):
        ...
        CONFIG_TARGET_DIR = f"{self.master.target_dir.rstrip(os.sep)}{version_suffix}"
